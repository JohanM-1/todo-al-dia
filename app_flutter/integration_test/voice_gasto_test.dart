// integration_test/voice_gasto_test.dart
//
// E2E test: Create a gasto using voice-simulated input.
//
// Flow: Home → tap FAB → AddMovementPage → enter amount → save → verify on home.
//
...
void main() {
  patrolTest(
    'create gasto via form (voice-simulated) and verify on home',
    ($) async {
      ...
      await $.enterText(amountField, '50.00');
      await $.pumpAndSettle();
  ...
  patrolTest(
    'create gasto with merchant and verify amount appears',
    ($) async {
      appRouter.go('/add');
      await $.pumpWidgetAndSettle(buildTestApp());
      ...
      // Amount
      final amountField = find.byType(TextFormField).first;
      await $.enterText(amountField, '125.50');
      await $.pumpAndSettle();

      // Merchant (second TextFormField)
      final merchantField = find.byType(TextFormField).at(1);
      await $.enterText(merchantField, 'Café del centro');
      await $.pumpAndSettle();
